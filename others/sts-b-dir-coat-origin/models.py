...
class HeadlessPairEncoder(Model):
    ...
    def forward(self, s1, s2, m1=None, m2=None):
        ...
        s1_enc_coat = self._phrase_layer['coat'](s1_embs.transpose(1,2), s1_lstm_mask)
        s2_enc_coat = self._phrase_layer['coat'](s2_embs.transpose(1,2), s2_lstm_mask)
        ...
        s1_enc_ranksim_mean = s1_enc_ranksim.sum(dim=1)/s1_mask.sum(dim=1)
        s2_enc_ranksim_mean = s2_enc_ranksim.sum(dim=1)/s2_mask.sum(dim=1)
        ...
        s1_enc_ranksim_max, _ = s1_enc_ranksim.max(dim=1)
        s2_enc_ranksim_max, _ = s2_enc_ranksim.max(dim=1)
        
        s1_enc_ranksim = torch.cat([s1_enc_ranksim_max,s1_enc_ranksim_mean],1)
        s2_enc_ranksim = torch.cat([s2_enc_ranksim_max,s2_enc_ranksim_mean],1)

        s1_enc = torch.cat([s1_enc_coat,s1_enc_ranksim],1)
        s2_enc = torch.cat([s2_enc_coat,s2_enc_ranksim],1)
