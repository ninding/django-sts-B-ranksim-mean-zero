...
class SamplingMultiTaskTrainer():
    ...
    def _setup_training(self, tasks, train_params, optimizer_params, iterator):
        ...
        for task in tasks:
            ...
            tr_generator = iterator(task.train_data, num_epochs=None) # by default, shuffle=True
            ...
            # print(train_params)
            # task_info['optimizer'] = torch.optim.Adam({ 'lr': train_params['lr'], 'weight_decay': 1e-5})
    ...
    def train(self, tasks, validation_interval, train_params, optimizer_params, resume=False):
        ...
        schedual,_= create_scheduler(self.args, task_infos[samples[n_pass % (validation_interval)].name]['optimizer'])
        epoch=0
        ...
        while not should_stop:
            ...
            for batch in itertools.islice(tr_generator, 1):
                ...
                optimizer.zero_grad()
                # print(batch.,task,real_epoch)
                # exit()
                # for x in batch['input1']:
                #     batch['input1'][x] = batch['input1'][x].cuda()
                # # batch['input1'] = batch['input1'].cuda()
                ...
                if self.args.regularization_weight > 0:
                    loss += (self.args.regularization_weight * batchwise_ranking_regularizer(output_dict['embs'], output_dict['labels'], 
                        self.args.interpolation_lambda))
    ...
    def from_params(cls, args, model, serialization_dir, iterator, params):
        ...
        return SamplingMultiTaskTrainer(args,
                                        model,
                                        iterator,
                                        patience=patience,
                                        max_vals=max_vals,
