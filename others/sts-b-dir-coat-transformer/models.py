...
def build_model(args, vocab, pretrained_embs, tasks):
    ...
    # Build encoders
    phrase_layer = {'ranksim':s2s_e.by_name('lstm').from_params(Params({'input_size': d_inp_phrase,
                                                             'hidden_size': d_hid_phrase,
                                                             'num_layers': args.n_layers_enc,
                                                             'bidirectional': True})).cuda(),
                                                             'coat':nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=300, nhead=6), num_layers=6).cuda()}
...
class MultiTaskModel(nn.Module):
    ...
    def forward(self, task=None, epoch=None, input1=None, input2=None, mask1=None, mask2=None, label=None, weight=None):
        ...
        pair_emb = self.pair_encoder(input1, input2, mask1, mask2, epoch)
        pair_emb_s = pair_emb
...
class HeadlessPairEncoder(Model):
    def __init__(self, vocab, text_field_embedder, num_highway_layers, phrase_layer,
        ...
        self.pad_idxc = vocab.get_token_index(vocab._padding_token)
    ...
    def forward(self, s1, s2, m1=None, m2=None, epoch=0):
        # print(s1,s2,m1,m2)
        ...
        print(s1_lstm_mask.shape)
        s1_lstm_mask_coat = torch.einsum("bi,bj->bij",[s1_lstm_mask,s1_lstm_mask])
        s2_lstm_mask_coat = torch.einsum("bi,bj->bij",[s2_lstm_mask,s2_lstm_mask])
        # print(s1_lstm_mask.shape,s1_lstm_mask_coat.shape)
        # exit()
        s1_enc_coat = self._phrase_layer['coat'](s1_embs, s1_lstm_mask_coat)
        s2_enc_coat = self._phrase_layer['coat'](s2_embs, s2_lstm_mask_coat)

        # Sentence encodings with LSTMs
        s1_enc_ranksim = self._phrase_layer['ranksim'](s1_embs, s1_lstm_mask)
        s2_enc_ranksim = self._phrase_layer['ranksim'](s2_embs, s2_lstm_mask)

        s1_enc_ranksim = self._dropout(s1_enc_ranksim)
        s2_enc_ranksim = self._dropout(s2_enc_ranksim)

        # Max pooling
        s1_mask = s1_mask.unsqueeze(dim=-1)
        s2_mask = s2_mask.unsqueeze(dim=-1)
        s1_enc_ranksim.data.masked_fill_(1 - s1_mask.byte().data, -float('inf'))
        s2_enc_ranksim.data.masked_fill_(1 - s2_mask.byte().data, -float('inf'))
        s1_enc_ranksim, _ = s1_enc_ranksim.max(dim=1)
        s2_enc_ranksim, _ = s2_enc_ranksim.max(dim=1)

        s1_enc = torch.cat([s1_enc_ranksim,s1_enc_coat],1)
        s2_enc = torch.cat([s2_enc_ranksim,s2_enc_coat],1)
