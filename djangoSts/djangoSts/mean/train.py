...
from djangoSts.mean.preprocess import build_tasks, myprocess_task
from djangoSts.mean.models import build_model
from djangoSts.mean.trainer import build_trainer
from djangoSts.mean.evaluate import evaluate
from djangoSts.mean.util import device_mapping, query_yes_no, resume_checkpoint
...
def main(s,mydata=None):
    ...
    if 'store_root' not in d:
        d['store_root'] = './djangoSts/mean/checkpoint'
    ...
    # exit()
    model_state = torch.load(model_path, map_location=device_mapping(args.cuda))
    # model_state['sts-b_pred_layer.weight']
