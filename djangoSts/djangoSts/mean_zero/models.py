...
from djangoSts.mean_zero.loss import *
from djangoSts.mean_zero.util import calibrate_mean_var
...
class HeadlessPairEncoder(Model):
    ...
    def forward(self, s1, s2, m1=None, m2=None):
        ...
        # s1_enc_mean = s1_enc.mean(dim=1)
        # s2_enc_mean = s2_enc.mean(dim=1)
        
        s1_enc.data.masked_fill_(1 - s1_mask.byte().data, 0)
        s2_enc.data.masked_fill_(1 - s2_mask.byte().data, 0)
        s1_enc_mean = s1_enc.sum(dim=1)/s1_mask.sum(dim=1)
        s2_enc_mean = s2_enc.sum(dim=1)/s2_mask.sum(dim=1)
        s1_enc.data.masked_fill_(1 - s1_mask.byte().data, -float('inf'))
        s2_enc.data.masked_fill_(1 - s2_mask.byte().data, -float('inf'))
        s1_enc_max, _ = s1_enc.max(dim=1)
        s2_enc_max, _ = s2_enc.max(dim=1)

        s1_enc = torch.cat([s1_enc_max,s1_enc_mean],1)
        s2_enc = torch.cat([s2_enc_max,s2_enc_mean],1)
        ...
        # s1_enc = torch.cat([s1_enc,torch.tensor(np.zeros((16,3000))).cuda()],1)
        # s2_enc = torch.cat([s2_enc,torch.tensor(np.zeros((16,3000))).cuda()],1)
